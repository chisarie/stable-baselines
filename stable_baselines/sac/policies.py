...
def mlp(input_ph, layers, activ_fn=tf.nn.relu, layer_norm=False, l2_reg=0.0):
    """
    ...
    for i, layer_size in enumerate(layers):
        output = tf.layers.dense(output, layer_size, name='fc' + str(i), kernel_regularizer=tf.contrib.layers.l2_regularizer(l2_reg))
        if layer_norm:
...
class FeedForwardPolicy(SACPolicy):
    ...
    def make_actor(self, obs=None, reuse=False, scope="pi", l2_reg=0.0):
        if obs is None:
        ...
        with tf.variable_scope(scope, reuse=reuse):
            ...
            pi_h = mlp(pi_h, self.layers, self.activ_fn, layer_norm=self.layer_norm, l2_reg=l2_reg)
